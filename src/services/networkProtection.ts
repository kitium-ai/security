...
export class NetworkProtectionService {
  public requireMutualTLS() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      if (!authorized) {
        auditLogService.logSecurityViolation(req.securityContext?.organizationId || 'unknown', 'mtls_failed', {
  ...
  public fileScanMiddleware(scanner: (buffer: Buffer) => Promise<boolean>) {
    return async (req: Request, res: Response, next: NextFunction) => {
      ...
      if (!clean) {
        auditLogService.logSecurityViolation(req.securityContext?.organizationId || 'unknown', 'file_scan_failed', {
          requestId: req.securityContext?.requestId,
        });
