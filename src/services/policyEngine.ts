...
export class PolicyEngineService {
  ...
  public middleware(requiredAction: string, resource: string, tenantResolver?: (req: Request) => string) {
    return (req: Request, res: Response, next: NextFunction) => {
      const tenant = tenantResolver ? tenantResolver(req) : req.securityContext?.organizationId || 'default';
  ...
  private runLocalEvaluation(context: PolicyDecisionContext, bundle: Record<string, any>): boolean {
    ...
    if (denyList.some((rule: any) => rule.action === context.action && rule.resource === context.resource)) {
      return false;
    }
