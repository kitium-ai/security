...
export class IdentityFederationService {
  ...
  public requireConsentScopes(requiredScopes: string[]) {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      if (missing.length) {
        logger.warn('Consent scopes missing', { requiredScopes: missing, requestId: context?.requestId });
        return res.status(403).json({ error: 'Consent scopes missing', requiredScopes: missing, requestId: context?.requestId });
      }
  ...
  public requireStepUp(requiredLevel: 'medium' | 'high') {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      const allowed = assurance === 'high' || (assurance === 'medium' && requiredLevel === 'medium');

      if (!allowed) {
        auditLogService.logSecurityViolation(context?.organizationId || 'unknown', 'step_up_required', {
